=== WC4BP -> Booking ===

Contributors: themekraft, svenl77, gfirem, marin250189
Tags: WooCommerce, Booking, Bookings, WooCommerce Bookings, BuddyPress, Shop, eCommerce, social networking, social shopping, customer, customer relation, achievements, support, product, vendor, marketplace, groups, support groups, profile, my account, my-account
Requires at least: 4.0
Tested up to: 4.9
Stable tag: 1.0.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

WooCommerce for BuddyPress Booking - Integrate BuddyPress with WooCommerce Bookings. Ideal for any subscription and membership sites using the WooCommerce Bookings Extension

== Description ==

BuddyPress WooCommerce Bookings Integration. With WC4BP Booking you can integrate the popular WooCommerce Bookings Plugin with BuddyPress.

### Let your Customers Manage there Bookings from there Profile.
* With WC4BP Booking you can integrate the My Bookings Account Page into the BuddyPress Member Profile Shop as new Tab.


### Videos
Overview over the plugin options [youtube https://www.youtube.com/watch?v=DtPk-WKo8ww&feature=youtu.be]


---

> #### Docs & Support
> * Find our Getting Started, How-to and Developer Docs on [docs.themekraft.com](http://docs.themekraft.com/)

---

> #### Submit Issues - Contribute
> * Pull request are welcome. WC4BP is community driven and developed on [Github](https://github.com/Themekraft/wc4bp)

---


> #### Follow Us
> [Blog](https://themekraft.com/blog/) | [Twitter](https://twitter.com/themekraft) | [Facebook](https://www.facebook.com/themekraft/)

---

> **Powered with ❤ by [ThemeKraft](https://themekraft.com)**

---

#### Tags
Tags: WooCommerce, BuddyPress, Shop, eCommerce, social networking, social shopping, customer, customer relation, achievements, support, product, vendor, marketplace, groups, support groups, profile, my account, my-account


== Installation ==
Upload the entire wc4bp-bookings folder to the /wp-content/plugins/ directory. Head to the 'WC4BP Settings' menu item in your admin sidebar to adjust the settings


== Changelog ==
= 1.0.0 20 Jun 2018 =
-> first public version